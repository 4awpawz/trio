...
const { log, normalizeFileName } = require("../utils");
...
const applyPermalink = (metadata, destinationPath) => {
    ...
    if (config.userConfig.permalinks) {
        ...
        // begining with v4.0.0 permalinks preserve the paths to child folders of targetFolder
        const path = match && join(destinationPath.replace(match.targetFolder, match.path)) || destinationPath;
        return path;
    }
...
module.exports = metadata => {
    ...
    const fileName = normalizeFileName(parse(metadata.path).base);
    if (parse(metadata.path).dir.startsWith(`${config.sourceArticles}`)) {
        ...
        const articlePath = getArticleDestPath(fileName);
        ...
        // all other pages
        destinationPath = parse(fileName).name === "index" &&
            join(applyPermalink(metadata, baseDir), "index.html") ||
            join(applyPermalink(metadata, join(baseDir, parse(fileName).name)), "index.html");
    }
