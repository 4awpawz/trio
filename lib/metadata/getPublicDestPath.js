...
module.exports = metadata => {
    if (metadata.path.indexOf(config.sourceBlog) !== -1) {
        ...
        if (metadata.path.indexOf(config.sourceArticles) !== -1) {
            // article
            const destPath = getArticleDestPath(parse(metadata.path).base);
            const relPath = relative(config.sourceArticles, metadata.path);
            return `${config.publicArticles}${sep}${parse(relPath).dir}${sep}${destPath}`;
        }
        if (metadata.path.indexOf(config.sourceTag) !== -1) {
            // tag
            const relPath = relative(config.sourceTag, metadata.path);
            return `${config.publicTag}${sep}${parse(relPath).dir}${sep}${parse(relPath).name}.html`;
        }
    ...
    //     // articles
    //     const destPath = getArticleDestPath(parse(metadata.path).base);
    //     const categoryPath = parse(relative(config.sourceArticles, metadata.path)).dir;
    //     return categoryPath
    //         ? `${config.publicArticles}${sep}${categoryPath}${sep}${destPath}`
    //         : `${config.publicArticles}${sep}${destPath}`;
    // }
