...
// by deconstructing their original name:
...
module.exports = metadata => {
    if (parse(metadata.path).dir.startsWith(`${config.sourceBlog}`)) {
        ...
        if (metadata.path.indexOf(config.sourceArchive) !== -1) {
            // archive
            const relPath = relative(config.sourceArchive, metadata.path);
            const destPath = parse(relPath).name === "index"
                ? `${config.publicArchive}${sep}index.html`
                : `${config.publicArchive}${sep}${parse(relPath).name}${sep}index.html`;
            return normalize(destPath).toLowerCase();
        }
        // all other blog pages (e.g. blog index pages)
        const relPath = relative(config.sourceBlog, metadata.path);
        const destPath = parse(relPath).name === "index"
            ? `${config.publicBlog}${sep}${sep}index.html`
            : `${config.publicBlog}${sep}${parse(relPath).name}${sep}index.html`;
        return normalize(destPath).toLowerCase();
    }
