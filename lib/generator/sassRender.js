...
module.exports = async () => {
    ...
    if (mainSassFile) {
        const generateSourceMap = process.env.TRIO_ENV_buildType === "release" &&
            config.userConfig.sassSourceMaps.release ||
            process.env.TRIO_ENV_buildType !== "release" &&
            config.userConfig.sassSourceMaps.development;
        ...
        const cssOutput = generateSourceMap && process.env.TRIO_ENV_buildType === "release" &&
                postcssResult.css.replace(`/*# sourceMappingURL=${parse(mainSassFile).name}.css.map */`,
                    `/*# sourceMappingURL=/css/${parse(mainSassFile).name}.css.map */`) || postcssResult.css;
        generateSourceMap && writeFileSync(sassMapFileName, postcssResult.map.toString());
