...
// ToDo: optimize incremental build (*** ignore trio.manifest.json for this and address it later as this has to take priority over maintaining that for now - you might want to make its generation a cli option, such as trio b --manifest ***):
//  1) use a stats map and its items as containers when geting all metadata and only obtain metadata for stale stats
//  2) serialize the stats map to .cache at the end of each build
//  3) respond to file add, change, delete events from file watcher by adding, changing and deleting stats in the stats map
//  4) when mashing up pages, use stats map to compose individual pages
//  5) rename mashup to compose to better reflect its purpose
// ToDo: rename the public folder to baseUrl?
...
module.exports = async () => {
    ...
    const assets = globFriendly(`${config.source}/**/*.*`, { dot: true });
