...
const pipeline = (frag, frags) => {
    ...
    if (frag.path.startsWith(`${config.fragments}${sep}blog${sep}articles`)) {
        const postCallback =
            requireUncached(`${process.cwd()}${sep}${config.callbacks}${sep}${config.postCallback}`);
        if (postCallback) {
            postCallback($, frag, frags);
        }
    }
    if (frag.callback) {
...
const generate = async () => {
    ...
    const siteMetaData = {
        ...
        sortedArticlesCatalog: createSortedArticlesCatalog(frags)
    };
