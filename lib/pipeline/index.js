    ...
    glob.sync(`${defConfig.fragments}/**.${type}`)
        ...
        .map(metaData => {
            return {
                ...metaData,
                ...{
                    pageName: metaData.matter.data.page,
                    pagePath: `${defConfig.pages}${sep}${metaData.matter.data.page}`,
...
//     content: type === "html"
//         ? matter.content.split("\n").join("")
//         : marked(matter.content).split("\n").join("")
// }));
...
const pipeline = metaData => {
    const $ = cheerio.load(read(metaData.pagePath));
    ...
    writeFileSync(`${defConfig.public}${sep}${metaData.name}`,
