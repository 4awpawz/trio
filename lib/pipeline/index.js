...
const getArticleCount = frags => frags.filter(frag =>
    frag.path.startsWith(config.sourceArticles)).length || 0;
...
const getPaginatedBlogPagesCount = articlesCount =>
    articlesCount <= config.options.paginate
        ? 0
        : parseInt((articlesCount - config.options.paginate) / config.options.paginate, 10) +
        (articlesCount % config.options.paginate ? 1 : 0);

const createPagainatedBlogPages = (frags, count) => {
    ...
    if (blogIndexFrag) {
        for (let i = 0; i < count; i++) {
            ...
            console.log("blogPageFrag", blogPageFrag);
...
const generate = async () => {
    ...
    createPagainatedBlogPages(frags, getPaginatedBlogPagesCount(getArticleCount(frags)));

    siteMetaData.frags.filter(frag => !frag.path.startsWith(config.sourceBlog))
        .forEach((fragmentMetaData) => pipeline(fragmentMetaData, siteMetaData));

    siteMetaData.frags.filter(frag => frag.path.startsWith(config.sourceArticles))
