...
const getPublicDestPath = metaData => {
    ...
    if (relPath) {
        ...
        return `${config.public}${sep}${parse(metaData.path).base}`;
    }
};

const getUrlFromPublicDestPath = path => path.substring(path.indexOf(`${sep}`));
    ...
    glob.sync(`${config.fragments}/**/{*.html,*.md}`)
        ...
        .map(metaData => ({
            ...
            ...{
                ...
                url: getUrlFromPublicDestPath(getPublicDestPath(metaData))
            }
        }));
...
const generate = async () => {
    ...
    writeJSONSync(`${process.cwd()}${sep}site.manifest.json`, siteMetaData, {spaces: "    "});
    sassRender();
    if (process.env.TRIO_ENV === "release") {
        await cacheBust();
    }
