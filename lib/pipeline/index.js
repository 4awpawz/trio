    ...
    glob.sync(`${config.fragments}/**/{*.html,*.md}`)
        ...
        .map(metaData => {
            return {
                ...metaData,
                ...{
                    matter: matter(metaData.fileContent, { delimiters: config.fmDelimiters })
                }
            };
        })
        .map(metaData => {
...
const pipeline = (fragmentMetaData, siteMetaData) => {
    ...
    if (config.options.removeDataAttributes) {
        removeDataAttributes($);
    }
    writeFileSync(fragmentMetaData.destPath, config.options.beautifyHTML ? beautify($.html()) : $.html());
};
