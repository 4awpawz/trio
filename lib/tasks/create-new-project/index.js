...
module.exports = (target, options) => {
    try {
        ...
        const msg = "*** Please enter the path to an empty folder and try again.";
        ...
        if (fs.existsSync(target) && globFriendly(join(target, "*.*")).length > 0) {
            log(`*** Error: The target folder "${target}" is not empty.`);
            log(msg);
            process.exit();
        }
        ...
        if (options.length === 0) {
            ...
            fs.ensureDirSync(`${target}${sep}source${sep}filters`);
