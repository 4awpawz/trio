...
module.exports = (assets) => {
    ...
    assets.filter(path => path.startsWith(config.fragments))
        .forEach(path => {
            ...
            // add ".html" to matter.data.template if user omitted it
            matter.data.template = getTemplateFileName(matter.data.template);
            ...
            // if generator check for missing filter callback
            matter.data.collection && matter.data.collection.filterFn &&
                noteMissingAsset(`${config.filters}${sep}${matter.data.collection.filterFn}.js`, path);
        });
